...
import { GITHUB_REPO_URL, SITE_ALTERNATE_NAME, SITE_NAME } from '@/lib/site-config';

export const metadata: Metadata = buildPageMetadata({
  title: `关于${SITE_NAME}`,
  description: `了解${SITE_NAME}为什么要先做 Soul 人格层、内容来源原则、投稿审核规则，以及联系与安全反馈方式。`,
  pathname: '/about',
  keywords: [`关于${SITE_NAME}`, SITE_ALTERNATE_NAME, '投稿规则', '来源与授权', '审核规则'],
});
  ...
  return (
    <>
      ...
      <main className="page-shell prose-page about-page">
        <p className="eyebrow">关于{SITE_NAME}</p>
        <h1>这是一个 OpenClaw Soul 人格分享平台，不只是堆 <code>SOUL.md</code> 的目录页</h1>
        <p className="prose-page__lead">
          {SITE_NAME} 想解决的不是“有没有 Soul”，而是“灵魂人格太散，看不清边界，也很难决定该不该替换进自己的龙虾”。所以它把零散的 <code>SOUL.md</code> 整理成一个可发现、可比较、可导入、可投稿的站点。
        </p>
        ...
        <section>
          ...
          <div className="prose-page__actions">
            <a href={`${GITHUB_REPO_URL}/issues`} target="_blank" rel="noreferrer" className="text-action-link">{SITE_ALTERNATE_NAME} GitHub 仓库 Issues</a>
            <a href={`${GITHUB_REPO_URL}/discussions`} target="_blank" rel="noreferrer" className="text-action-link">{SITE_ALTERNATE_NAME} GitHub 仓库 Discussions</a>
            <a href={`${GITHUB_REPO_URL}/blob/main/SECURITY.md`} target="_blank" rel="noreferrer" className="text-action-link">{SITE_NAME} 安全反馈说明</a>
          </div>
        </section>
